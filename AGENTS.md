...
Gerador via Gemini API. 23 tópicos rotativos (do mais simples ao mais complexo): WhatsApp, landing pages, Python, Telegram, Instagram, gestão de tráfego e redes sociais, análise de mercado/produtos, micro e grandes empresas, etc. **Rotação sequencial por dia** (1 tema/dia, por ordem, começando a 1 Ago 2026) — não aleatória. Sem repetição de artigos já publicados. Invoca `post.sh`. **Robustez:** tenta 4 modelos × 2 versões da API com retry; captura HTTPError, URLError e timeouts. **Validação de qualidade:** `validate_content()` corre antes de publicar — rejeita artigos truncados (tags <pre> partidas, conteúdo cortado a meio), com comentários HTML, com <h1> no corpo ou com o título repetido no primeiro <h2>; tenta gerar de novo e aborta se falhar 2×. Autor: "Marcus" (nunca MARC-Jarvis nem "Hermes Agent").
- **Autoria humana (19 Ago):** a partir de agora os artigos têm **autoria humana "Marcus"** (byline `✍️ Marcus`, `<meta name="author" content="Marcus">`, JSON-LD `"author": {"@type": "Person", "name": "Marcus", "url": "https://github.com/Jorgeacus"}`) — nunca "Hermes Agent" — porque o AdSense exige autor real verificável. O `post.sh` (default) e o `generate-post.py` já usam "Marcus". Os 34 artigos existentes foram atualizados em massa.
- **Sem CTA de venda no corpo dos artigos (19 Ago):** o bloco `article-cta` (e links `hotm.io`) foram removidos do corpo de todos os artigos — o CTA do livro fica apenas na sidebar (widget "Livro do Hermes Agent") e no artigo de lançamento. O `post.sh` já não injeta o bloco; o `generate-post.py` só permite uma menção discreta ao ebook com link `/livro/` (sem preço nem urgência).
- **Comentários Giscus (19 Ago):** todos os artigos têm uma secção `<div class="comments-section">` com o widget Giscus (repo `Jorgeacus/marctechja-blog`, categoria "General", mapping `pathname`, lang `pt`). Requisitos: Discussions ativado no repo (feito 19 Ago via API) + app giscus autorizada em https://github.com/apps/giscus. O template do `post.sh` já inclui o bloco.
...
### Estrutura didática
1. **Introdução curta** que desperta interesse antes do primeiro heading — variar a estrutura (evitar repetir o mesmo gancho "Sentes que..." em todos os artigos; alternar entre perguntas, declarações diretas e cenários concretos).
...
## Pendente
- ⬜ **AdSense** — **2ª revisão reprovada (14 Ago, email genérico "site not ready"; publisher `pub-3717814491008089`).** Corrigido (19 Ago) com base na pesquisa das políticas Google 2026: **autoria humana "Marcus"** em todos os 34 artigos (byline, meta author, JSON-LD com url GitHub); **aberturas diversificadas** (removidos os 19 ganchos "Sentes que..." — sinal de conteúdo escalado); **CTAs de venda removidos do corpo** (bloco `article-cta` + links `hotm.io` → apenas sidebar + artigo de lançamento); **tempo de leitura corrigido** (marcador adicionado aos artigos que não tinham, valores recalculados por contagem de palavras); **secção de comentários Giscus** adicionada a todos os artigos (Discussions ativado no repo 19 Ago; falta autorizar a app giscus em https://github.com/apps/giscus). **Fator de tempo:** o site tinha ~3 semanas na 2ª rejeição — a pesquisa indica que o mínimo prático é 2-3 meses de publicação ativa; considerar pedir nova revisão após esse período. Histórico (4 Ago): páginas legais + CMP + artigos reforçados para ≥800 palavras.
  - **Descobertas da pesquisa AdSense (4 Ago + 19 Ago):**
    - A meta tag `<meta name="google-adsense-account" content="ca-pub-...">` É o método recomendado de código AdSense — já está em todas as páginas. NÃO adicionar o script `adsbygoogle.js`/unidades de anúncio até a revisão aprovar.
    - Revisão "site not ready" foca: conteúdo único/original, navegação clara, UX; Google **sugere secção de comentários** (moderada) — implementada via Giscus (19 Ago).
    - "Insufficient content" / "content quality": páginas precisam de texto suficiente, frases completas, site totalmente construído; conteúdo original e rico; **não** colocar anúncios em páginas auto-geradas. Padrões repetitivos em massa ("Sentes que...", CTAs idênticos, autor IA) são sinais de scaled content abuse (política 2026).
  ...
  - **Melhorias adicionais (4 Ago, auditoria profunda):** JSON-LD (WebSite/Organization/Organization+BlogPosting) em home, sobre e todos os artigos + template post.sh; página Sobre com identidade do autor (Marcus/GitHub Jorgeacus); Privacidade+Cookies com `aboutads.info` e menção ao banner; divulgação de afiliados nos Termos (4.1) e no livro; `<lastmod>` no sitemap; link "Definições de cookies" no rodapé (reabre o banner via `window.MarctechJACookieConsent.show()`).
- ⬜ Exportar subscritores da Sheet para `subscribers.csv` quando houver dados

## MARC-Jarvis (agente local) — papel no blog
- **Papel:** Editor/Consultor/Administrador (a autoria nos artigos é "Marcus" — ver acima; nunca "Hermes Agent").
- **Regras gravadas em:** `memory/long_term.json` (Mark-XLVII) + `core/prompt.txt` (protocolo).
- **Regras:** 1 artigo/dia ao ligar o Mac; nunca assumir "novo dia" só por reinício; consultar `notes.blog_artigos_publicados` antes de gerar (não repetir temas); formato/cores Hermes intocáveis; CTA ebook na sidebar (não no corpo) + subscrição; páginas legais; AdSense `ca-pub-3717814491008089`; autoria "Marcus".
