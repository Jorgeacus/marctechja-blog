...
- **Falha de publicação (6 Ago)**: o run diário falhou sem publicar. Causa: `generate-post.py` gerava artigos truncados (limite `maxOutputTokens: 4096` cortava tutoriais com muito código, deixando `<pre>` a meio) e a validação só tinha 2 tentativas. **Correção:** `maxOutputTokens: 8192`, até **3 tentativas** de regeneração na validação, e guarda que salta tópicos já publicados em `blog/` (evita duplicar se o artigo do dia já tiver sido publicado manualmente).
